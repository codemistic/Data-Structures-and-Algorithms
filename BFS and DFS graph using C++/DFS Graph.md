## Depth first search (DFS) algorithm starts with the starting node, and then travers each branch of the graph until we find the leaf node which is a node that has no children.
```
#include <iostream>
#include<vector>
using namespace std;
